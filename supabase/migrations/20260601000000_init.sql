...
CREATE TABLE IF NOT EXISTS public.audit_logs (
  ...
  description text,
  created_at timestamptz default now(),
  -- Server-stamped, unforgeable. user_id above is the app's (spoofable) guess at
  -- the actor; created_by is who actually uploaded the row (auth.uid()), enforced
  -- by the INSERT policy below. Trust created_by for audit integrity.
...
-- SECURITY: role is intentionally NEVER read from client-supplied signup
-- metadata -- every new account starts as 'viewer'. Elevated roles are
-- granted afterwards by an existing admin, or via claim_first_admin() for
-- the very first mosque admin. Trusting raw_user_meta_data->>'role' here
-- previously let anyone sign up requesting role=admin directly.
...
-- One-time, atomic "become the first admin" claim used by the mosque
-- registration screen. Succeeds only if no admin exists yet anywhere in
-- this project, so it can never be used to create a second admin account.
...
-- Public, side-effect-free check so the registration screen can show a
...
-- Users can update their own profile, but never their own `role` column
-- (WITH CHECK pins role to its current value; without this, USING alone
-- only restricts which row can change, not which columns).
...
-- Insert: authenticated users can insert logs, but created_by must equal the
-- caller (auth.uid()). The client never sends created_by, so the column default
-- stamps it automatically and this check passes; an attempt to forge it as
-- someone else is rejected. Trust created_by, not the free-form user_id.
DROP POLICY IF EXISTS "Authenticated can insert logs" ON public.audit_logs;
CREATE POLICY "Authenticated can insert logs" ON public.audit_logs FOR INSERT WITH CHECK (auth.role() = 'authenticated' AND created_by = auth.uid());
...
-- 8. Amount/duration sanity CHECK constraints -- previously only enforced
-- in the Flutter UI, so any other writer (bulk import, a direct API call,
-- a future admin tool) could still store 0/negative amounts. NOT VALID
-- skips validating pre-existing rows (safe to run against a live project
-- that may already have data), while still enforcing the check on every
-- future insert/update; run VALIDATE CONSTRAINT separately once you've
-- confirmed existing data is clean.
